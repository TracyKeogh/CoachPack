/*
  ...
  1. New Tables
    - `wheel_signups` - Stores free assessment signups
...
-- Create enum types for subscription statuses (with idempotent checks)
DO $$ 
BEGIN
...
CREATE INDEX IF NOT EXISTS wheel_signups_email_idx ON public.wheel_signups(email);
CREATE INDEX IF NOT EXISTS wheel_signups_created_at_idx ON public.wheel_signups(created_at);
...
CREATE UNIQUE INDEX IF NOT EXISTS user_wheel_data_user_id_key ON public.user_wheel_data(user_id);
CREATE INDEX IF NOT EXISTS user_wheel_data_user_id_idx ON public.user_wheel_data(user_id);
...
CREATE UNIQUE INDEX IF NOT EXISTS user_values_data_user_id_key ON public.user_values_data(user_id);
CREATE INDEX IF NOT EXISTS user_values_data_user_id_idx ON public.user_values_data(user_id);
...
CREATE UNIQUE INDEX IF NOT EXISTS user_vision_data_user_id_key ON public.user_vision_data(user_id);
CREATE INDEX IF NOT EXISTS user_vision_data_user_id_idx ON public.user_vision_data(user_id);
...
CREATE UNIQUE INDEX IF NOT EXISTS user_goals_data_user_id_key ON public.user_goals_data(user_id);
CREATE INDEX IF NOT EXISTS user_goals_data_user_id_idx ON public.user_goals_data(user_id);
...
CREATE INDEX IF NOT EXISTS user_notes_related_feature_idx ON public.user_notes(related_feature);
...
CREATE UNIQUE INDEX IF NOT EXISTS stripe_customers_user_id_key ON public.stripe_customers(user_id);
...
-- Create views for easier access to user data (drop if exists first)
