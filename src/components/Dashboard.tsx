...
const Dashboard: React.FC<DashboardProps> = ({ onNavigate }) => {
  ...
  const getUserStory = () => {
    ...
    const overallProgress = Math.round((
      (wheelStats.wheelCompleted ? (wheelStats.allReflectionsCompleted ? 100 : 75) : (wheelStats.averageScore > 0 ? 50 : 0)) +
      (visionItems.length > 0 ? Math.min(100, (visionItems.length / 4) * 100) : 0) +
      goalsProgress.percentage +
      0 // tracking progress placeholder
    ) / 4);
